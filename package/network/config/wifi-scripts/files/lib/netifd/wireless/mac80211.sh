...
mac80211_prepare_vif() {
	...
	json_get_vars _device_id ifname mode ssid wds powersave macaddr enable wpa_psk_file vlan_file wifi_id

	[ -n "$ifname" ] || {

		# If for whatever reason for some interfaces device id would be missing
		# create dynamic names as previously but with different naming template
		# to avoid conflicts with static ones
		ifname="wlan${phy#phy}-${_device_id:-"D-$if_idx"}"
	}
...
mac80211_reset_config() {
	...
	ubus_call hostapd config_set '{ "phy": "'"$phy"'", "config": "", "prev_config": "'"$hostapd_conf_file"'" }' > /dev/null 2>&1
...
drv_mac80211_setup() {
	...
	local ant_gain_file
	ant_gain_file="/sys/kernel/debug/ieee80211/$phy/antenna_gain"

	[ -e "$ant_gain_file" ] && echo "$antenna_gain" > "$ant_gain_file"
