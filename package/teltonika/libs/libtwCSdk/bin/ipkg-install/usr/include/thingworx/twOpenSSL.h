...
static INLINE const char* TW_SSL_VERSION()
{
	...
	const char* raw_tls_full_version;
#if OPENSSL_VERSION_NUMBER >= 0x30000000L
	raw_tls_full_version = OpenSSL_version(OPENSSL_VERSION);

	if(strncmp(OpenSSL_version(OPENSSL_VERSION), OPENSSL_VERSION_TEXT,64) != 0){
		TW_LOG(TW_WARN, "TW_SSL_VERSION: Error tls runtime version: %s, does not "
				"match compiled version: %s",
				OpenSSL_version(OPENSSL_VERSION),OPENSSL_VERSION_TEXT);
	}
#else
	raw_tls_full_version = SSLeay_version(SSLEAY_VERSION); /* eg "OpenSSL 1.0.2l-fips  25 May 2017" */
...
static INLINE SSL_CTX * TW_NEW_SSL_CTX_FUNC() {
	...
	if (ctx) {
		...
		if (ret != 1) {
			TW_LOG(TW_ERROR,"TW_NEW_SSL_CTX_FUNC: Setting SSL cipher string Error: %s", ERR_reason_error_string(ERR_get_error()));
			TW_SSL_CTX_FREE(ctx);
			return NULL;
			}
		}
		...
		if (ret != TW_OK ) {
			TW_LOG(TW_ERROR,"TW_NEW_SSL_CTX_FUNC: Error getting valid cipher list: %s", ERR_reason_error_string(ERR_get_error()));
		}
	return ctx;
}
