...
struct tag {
	...
	char *source; /**< Source service (e.g., "dnp3_client") */
	char *description; /**< Optional description */
	tag_type_t type; /**< Data type */
	tag_value_t value; /**< Current value */
	size_t value_size; /**< Size of the value in bytes. Known on tag creation. */
	...
	tag_error_t (*read_cb)(tag_t *tag, void *user_data); /**< Read callback function */
	tag_error_t (*write_cb)(tag_t *tag, tag_value_t value, void *user_data); /**< Write callback function */
...
// /**
//  * @brief Register a read callback for a tag
//  *
//  * @param tm Pointer to the tag manager
...
//  */
// int tag_register_read_cb(tag_manager_t *tm, const char *tag_id, tag_read_cb_t cb, void *user_data);

// /**
//  * @brief Register a write callback for a tag
//  *
//  * @param tm Pointer to the tag manager
//  * @param tag_id ID of the tag
//  * @param cb Callback function
...
//  * @return int 0 on success, negative value on error
//  */
// int tag_register_write_cb(tag_manager_t *tm, const char *tag_id, tag_write_cb_t cb, void *user_data);
 ...
 *
 * Important: if multiple tags share the same ID, this function returns only the first one encountered.
 *
 ...
 */
tag_t *tag_read(tag_t *tag, void *user_data);
 ...
 * @return tag_t* Pointer to the tag, or NULL if not found or error
 */
tag_t *tag_write(tag_t *tag, tag_value_t value, void *user_data);
 ...
 * @return tag_t* Pointer to the tag, or NULL if not found or error
 */
tag_t *tag_update(tag_t *tag, tag_value_t value);
 ...
 * @return tag_t* Pointer to the tag, or NULL if not found or error
 */
tag_t *tag_write_by_id(tag_manager_t *tm, const char *tag_id, tag_value_t value, void *user_data);
 ...
 * @return tag_t* Pointer to the tag, or NULL if not found or error
 */
tag_t *tag_update_by_id(tag_manager_t *tm, const char *tag_id, tag_value_t value);
...
/**
 * @brief Convert tag to blob for ubus
 *
 * @param tag Pointer to the tag
 * @param b Pointer to the blob buffer
 * @return tag_error_t TAG_SUCCESS on success, error code on failure
 *         - TAG_ERROR_INVALID_PARAMETER: Invalid tag or blob buffer
 */
 ...
 */
void tag_free_value(tag_value_t *value);
 ...
 * @param user_data User data to pass to the callback
 * @return tag_error_t TAG_SUCCESS on success, error code on failure
 *         - TAG_ERROR_INVALID_PARAMETER: Invalid tag manager or callback
 *         - Other error codes may be returned by the callback function
 */
 ...
 * @return tag_error_t TAG_SUCCESS on success, error code on failure
 *         - TAG_ERROR_INVALID_PARAMETER: Invalid tag manager, filter or results array
 *         - TAG_SUCCESS with non-zero return: More results available than max_results
 */
...
/**
 * @brief Write a tag value to ubus (to other services)
 *
 * @param tag Pointer to the tag
 * @param value Value to write
 * @param ctx Pointer to the ubus context
 * @return tag_t* Pointer to the tag with updated value, or NULL if not found or error
 */
tag_t *tag_write_value_to_ubus(tag_t *tag, tag_value_t value, struct ubus_context *ctx);
