/* eslint-disable max-classes-per-file, prefer-destructuring, import/no-dynamic-require */
...
const ggDaemonPort = envVars.GG_DAEMON_PORT;
...
function _compareVersion(sdkIV, maxIV) {
    ...
    return !(sdkMajorVersion !== maxMajorVersion || sdkMinorVersion > maxMinorVersion);
}
...
class Invocation {
    ...
    invoke(customerHandler, customerModule, event) {
        ...
        try {
            const cb = this.handlerCallback.bind(this);
            const res = customerHandler.apply(customerModule, [event, this.context, cb]);
            if (res instanceof Promise) {
                res.then((data) => cb(undefined, data))
                    .catch(throwError);
            }
        } catch (e) {
            throwError(e);
...
class Runtime {
    ...
    // postWorkResultOrError - callback for when the customer wants to post their result or error
    _getNewInvocation(invocationId, clientContext) {
        return new Invocation(this._functionArn, invocationId, clientContext, this.postWorkResultOrError.bind(this));
    }
