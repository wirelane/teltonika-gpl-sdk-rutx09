...
static const char *ret_code_descript[] = { "clock synchronized",       "insert leap second",
					   "delete leap second",       "leap second in progress",
					   "leap second has occurred", "clock not synchronized" };
...
int main(int argc, char **argv)
{
	...
	if (!quiet && ret >= 0) {
		...
		printf(" )\n"
		       ...
		       "    time.tv_usec: %" PRId64 "\n"
		       "    return value: %d (%s)\n",
		       txc.constant, txc.precision, txc.tolerance, txc.tick, (int64_t)txc.time.tv_sec,
		       (int64_t)txc.time.tv_usec, ret,
