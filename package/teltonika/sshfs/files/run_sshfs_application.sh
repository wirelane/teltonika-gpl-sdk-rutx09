...
mkdir -p "/tmp/sshfs" -m 775
mkdir -p "/tmp/sshfs$mount_point" -m 775
echo "$password" | $PROG ${port:+-p $port} -o ssh_command='ssh -y' "$username@[$hostname]:$mount_path" "/tmp/sshfs$mount_point" -o password_stdin -o allow_other
