...
#define DRIVER_DESC \
	"USB serial driver for ch342/ch343/ch344/ch347/ch9101/ch9102/ch9103/ch9104, etc."
			    ...
			    u16 index, char *buf, unsigned bufsize)
{
	...
	retval =
		usb_control_msg(ch343->dev, usb_rcvctrlpipe(ch343->dev, 0),
				request,
...
static int ch343_get_status(struct ch343 *ch343)
{
	...
	retval = ch343_control_in(ch343, CMD_R, CMD_C3 + ch343->iface, 0,
				  buffer, size);
	if (retval != size)
		goto out;
...
static int ch343_configure(struct ch343 *ch343)
{
	...
	dev_dbg(&ch343->data->dev,
		...
		buffer[0], buffer[1], ch343->chiptype);
out:
	kfree(buffer);
	return r < 0 ? r : 0;
}
...
static int ch343_tty_write(struct tty_struct *tty, const unsigned char *buf,
			   int count)
{
	struct ch343 *ch343 = tty->driver_data;
	int stat;
...
static int ch343_get(CHIPTYPE chiptype, unsigned int bval, unsigned char *fct,
		     unsigned char *dvs)
{
	unsigned char a;
	unsigned char b;
	...
	if (((chiptype == CHIP_CH347T) || (chiptype == CHIP_CH344Q) ||
...
static int __init ch343_init(void)
{
	...
	ch343_tty_driver->init_termios.c_cflag =
