...
static int ch343_control_in(struct ch343 *ch343, u8 request, u16 value, u16 index, char *buf,
			    unsigned bufsize)
{
	int retval;
...
static int ch343_control_msg_out(struct ch343 *ch343, u8 request, u8 requesttype, u16 value, u16 index,
				 void *buf, unsigned bufsize)
{
	...
	if (retval)
		goto out;
	retval = usb_control_msg(ch343->dev, usb_sndctrlpipe(ch343->dev, 0), request, requesttype, value,
				 index, buffer, bufsize, DEFAULT_TIMEOUT);
	usb_autopm_put_interface(ch343->control);
...
static int ch343_control_msg_in(struct ch343 *ch343, u8 request, u8 requesttype, u16 value, u16 index,
				void *buf, unsigned bufsize)
{
	int retval;
	char *buffer;
	...
	if (retval)
		goto out;
	retval = usb_control_msg(ch343->dev, usb_rcvctrlpipe(ch343->dev, 0), request, requesttype, value,
				 index, buffer, bufsize, DEFAULT_TIMEOUT);
	if (retval > 0) {
...
static void ch343_update_status(struct ch343 *ch343, unsigned char *data, size_t len)
{
	...
	if (ch343->chiptype == CHIP_CH344L) {
		if (data[0] != 0x00)
			return;
		type = data[1];
	} else if (ch343->chiptype == CHIP_CH346C_M0 || ch343->chiptype == CHIP_CH346C_M1 ||
		   ch343->chiptype == CHIP_CH346C_M2 || ch343->chiptype == CHIP_CH339W ||
		   ch343->chiptype == CHIP_CH347TF || ch343->chiptype == CHIP_CH344Q ||
		   ch343->chiptype == CHIP_CH344L_V2 || ch343->chiptype == CHIP_CH9104L ||
		   ch343->chiptype == CHIP_CH9114L || ch343->chiptype == CHIP_CH9114F ||
		   ch343->chiptype == CHIP_CH9114W || ch343->chiptype == CHIP_CH9111L_M0 ||
		   ch343->chiptype == CHIP_CH9111L_M1) {
		type = data[1];
	}
...
#endif
{
	...
	int res;
	int old = ch343->ctrlout;

#ifdef IGNORE_RTSDTR
	return;
#endif

	// CH343_CTO_A - Automatic Flow Control Enable
	// CH343_CTO_D - DTR (Data Terminal Ready) Control
	// CH343_CTO_R - RTS (Request To Send) Control
	// All control parameters should be set and unset in one place
	// to prevent floating lines errors

	if (raise)
		ch343->ctrlout |= CH343_CTO_D | CH343_CTO_R | CH343_CTO_A;
	else
		ch343->ctrlout &= ~(CH343_CTO_D | CH343_CTO_R | CH343_CTO_A);

	if (ch343->ctrlout == old)
		return;
...
static int ch343_port_activate(struct tty_port *port, struct tty_struct *tty)
{
	...
	if (ch343->chiptype == CHIP_CH9114L || ch343->chiptype == CHIP_CH9114F ||
	    ch343->chiptype == CHIP_CH9114W || ch343->chiptype == CHIP_CH346C_M2) {
		retval = ch343_control_out(ch343, CMD_C8, 0x01 | (ch343->iface << 8), 0x00);
		if (retval) {
			goto error_submit_read_urbs;
		}
	}
...
static void ch343_port_shutdown(struct tty_port *port)
{
	...
	if (ch343->chiptype == CHIP_CH9114L || ch343->chiptype == CHIP_CH9114F ||
	    ch343->chiptype == CHIP_CH9114W || ch343->chiptype == CHIP_CH346C_M2) {
		r = ch343_control_out(ch343, CMD_C8, 0x02 | (ch343->iface << 8), 0x00);
		if (r)
...
retry:
	...
	if (wbn < 0) {
		spin_unlock_irqrestore(&ch343->write_lock, flags);
		timeout = wait_event_interruptible_timeout(ch343->sendioctl, ch343_wb_is_avail(ch343),
...
static int ch343_tty_break_ctl(struct tty_struct *tty, int state)
{
	...
	if (state != 0) {
		if (ch343->chiptype == CHIP_CH346C_M0 || ch343->chiptype == CHIP_CH346C_M1 ||
		    ch343->chiptype == CHIP_CH346C_M2 || ch343->chiptype == CHIP_CH339W ||
		    ch343->chiptype == CHIP_CH347TF || ch343->chiptype == CHIP_CH344L ||
		    ch343->chiptype == CHIP_CH344Q || ch343->chiptype == CHIP_CH344L_V2 ||
		    ch343->chiptype == CHIP_CH9104L || ch343->chiptype == CHIP_CH9114L ||
		    ch343->chiptype == CHIP_CH9114F || ch343->chiptype == CHIP_CH9114W ||
		    ch343->chiptype == CHIP_CH9111L_M0 || ch343->chiptype == CHIP_CH9111L_M1) {
...
static int ch343_get(struct ch343 *ch343, enum CHIPTYPE chiptype, unsigned int bval, unsigned char *fct,
		     unsigned char *dvs)
{
	unsigned char a;
	unsigned char b;
	...
	if ((ch343->chiptype >= CHIP_CH9111L_M0 && ch343->chiptype <= CHIP_CH346C_M2) && bval >= 2000000) {
		if ((ch343->chiptype >= CHIP_CH9111L_M0 && ch343->chiptype <= CHIP_CH346C_M2) &&
		    (bval > 13000000)) {
	...
	if (chiptype == CHIP_CH9114L || chiptype == CHIP_CH9114F || chiptype == CHIP_CH9114W ||
	    chiptype == CHIP_CH346C_M2) {
retry:
		...
		if (!change) {
			if ((gfreq == 120000000 && bval > 1000000 && (15000000 % bval)) ||
			    (gfreq == 96000000 && bval > 1000000 && (12000000 % bval)) ||
			    (gfreq == 80000000 && bval > 1000000 && (10000000 % bval))) {
				if (bfirst) {
					r = -EINVAL;
					goto out;
				} else {
					...
					r = ch343_control_out(ch343, CMD_C8, 0x01 | (ch343->iface << 8),
							      0x00);
					if (r) {
						dev_err(&ch343->control->dev, "%s - failed: %d\n", __func__,
							r);
...
#endif
{
	...
	if (r) {
		dev_err(&ch343->control->dev, "%s - Bad termios setting, DTERate: %d.\n", __func__,
			newline.dwDTERate);
		return;
	}
	...
	// CH343_CTO_A removed to avoid DTR/RTS errors
	// if (C_CRTSCTS(tty)) {
...
static int ch343_proc_show(struct seq_file *m, void *v)
{
	...
	for (i = 0; i < CH343_TTY_MINORS; ++i) {
		...
		seq_printf(m, " vendor:%04x product:%04x", le16_to_cpu(ch343->idVendor),
...
static void ch343_read_buffers_free(struct ch343 *ch343)
{
	...
	for (i = 0; i < ch343->rx_buflimit; i++)
		usb_free_coherent(usb_dev, ch343->readsize, ch343->read_buffers[i].base,
...
next_desc:
	...
	usb_fill_int_urb(ch343->ctrlurb, usb_dev, usb_rcvintpipe(usb_dev, epctrl->bEndpointAddress),
...
static const struct usb_device_id ch343_ids[] = { /* ch342 chip */
...
static int __init ch343_init(void)
{
	...
	ch343_tty_driver->init_termios.c_cflag = B200 | CS8 | CREAD | HUPCL | CLOCAL;
